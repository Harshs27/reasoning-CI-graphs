...
from scripts.uGLAD.glad.glad_params import glad_params
from scripts.uGLAD.glad import glad
from scripts.uGLAD.utils.metrics import reportMetrics

import scripts.uGLAD.utils.prepare_data as prepare_data
...
class uGLAD_GL(object):
    ...
    def fit(
        ...
        if mode=='missing':
            ...
            print(f'Direct Mode')
            pred_theta, compare_theta, model_glad, loss = run_uGLAD_direct(
                Xb,
                trueTheta=true_theta_b,
                eval_offset=eval_offset,
                EPOCHS=epochs, 
...
class uGLAD_multitask(object):
    ...
    def fit(
        ...
        start = time()
        # Running the uGLAD model
        pred_theta, compare_theta, model_glad, loss = run_uGLAD_multitask(
            Xb,
            trueTheta=true_theta_b,
            eval_offset=eval_offset,
            EPOCHS=epochs, 
...
def run_uGLAD_direct(
    ...
    return predTheta, compare_theta, model_glad, loss.detach().numpy()
